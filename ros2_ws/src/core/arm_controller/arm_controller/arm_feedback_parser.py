...
class ArmFeedbackParser(Node):
    ...
    def on_feedback(self, msg: ArmString) -> None:
        ...
        out.current = self._extract_currents(parsed)
    ...
    def _extract_angles(self, parsed: Dict[str, Any]) -> List[float]:
        return self._extract_series(
            parsed,
            array_keys=("angle_deg", "angles", "angle"),
            scalar_patterns=("servo{}_data", "angle{}", "joint{}", "servo{}"),
            single_value_keys=("angle",),
        )

    def _extract_currents(self, parsed: Dict[str, Any]) -> List[float]:
        return self._extract_series(
            parsed,
            array_keys=("current", "currents", "current_ma", "current_milliamp"),
            scalar_patterns=("current{}", "current{}_ma", "motor{}_current"),
            single_value_keys=("current",),
        )
        ...
        for obj in containers:
            for array_key in array_keys:
                ...
                if isinstance(maybe_array, (list, tuple)):
                    return self._normalize_series(maybe_array)
            ...
            for key in single_value_keys:
                if key not in obj:
                    continue
                values = [0.0] * 7
                values[idx] = _to_float(obj[key], 0.0)
                return values

        return [0.0] * 7

    def _normalize_series(self, values: Any) -> List[float]:
        result = [0.0] * 7
