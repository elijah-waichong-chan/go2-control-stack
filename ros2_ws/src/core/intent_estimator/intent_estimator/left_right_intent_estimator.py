...
class LeftRightIntentEstimatorNode(Node):
    ...
    def _update_status(self) -> None:
        ...
        if missing_topics:
            ...
            if self.waiting_on != waiting_topics:
                self.get_logger().info(
                    f"left_right_intent_estimator waiting for {waiting_topics}..."
                )
    ...
    def on_lowstate(self, msg: LowState) -> None:
        ...
        try:
            ...
            if len(msg.motor_state) <= max_index:
                raise ValueError(
                    "Expected /lowstate motor_state to include at least "
                    f"{max_index + 1} entries, got {len(msg.motor_state)}"
                )
