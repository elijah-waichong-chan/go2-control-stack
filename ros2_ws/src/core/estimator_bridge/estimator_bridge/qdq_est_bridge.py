...
class QdqEstBridge(Node):
    def __init__(self) -> None:
        ...
        self.pub_qdq = self.create_publisher(QDq, self.qdq_topic, 10)
        self.sub_odom = self.create_subscription(
            Odometry, self.odom_topic, self.on_odom, sensor_qos
        )
        self.sub_joint = self.create_subscription(
            JointState, self.joint_states_topic, self.on_joint, sensor_qos
        )
